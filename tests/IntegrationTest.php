...
it('can fetch edition data correctly', function () {
    ...
    expect($edition->legacyId)->toBeInt(244940);
    expect($edition->title)->toBe('Murtuneet mielet');
    expect($edition->ean)->toBeInt(9789510374665);
    ...
    expect($edition->costCenter->id)->toBeInt(313);
    ...
    expect($edition->contributors)->toBeInstanceOf(DataCollection::class)
        ...
        ->and($edition->contributors->first()->role->id)->toBeInt(326);
    ...
    expect($edition->mainGroup)->toBe('Tietokirjallisuus');
    expect($edition->subGroup)->toBe('Historia');
    expect($edition->depth)->toBeFloat(37.0000);
    expect($edition->height)->toBeFloat(218.0000);
    expect($edition->weight)->toBeFloat(0.6150);
    expect($edition->width)->toBeFloat(140.000);
    expect($edition->originalLanguages)->toBeEmpty();
    expect($edition->pages)->toBeInt(475);
    ...
    expect($edition->prices)->toBeInstanceOf(DataCollection::class)
        ...
        ->and($edition->prices->first()->value)->toBeFloat(41.32)
